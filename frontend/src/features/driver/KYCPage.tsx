...
export function KYCPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast(err.response?.data?.error || err.response?.data?.message || 'Upload failed', 'error');
    } finally {
      setUploading(false);
    }
  };
  ...
  if (loadingStatus) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-sm text-muted-foreground">Retrieving verification records...</p>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto space-y-6 py-12 px-4 text-center">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-3xl font-extrabold tracking-tight">Identity Verified</h1>
            <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mt-2">
              Congratulations! Your KYC verification has been approved. You now have full access to request, accept, and manage rentals.
            </p>
          </div>
          <div className="pt-4">
            <Button size="lg" onClick={() => navigate('/driver/bookings')} className="px-8 gap-2">
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6 py-12 px-4 text-center">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-3xl font-extrabold tracking-tight">Verification Pending</h1>
            <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mt-2">
              Your identity documents have been submitted and are currently under review by our administration.
            </p>
            <p className="text-xs text-muted-foreground bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50 rounded-xl px-4 py-2.5 max-w-xs mx-auto mt-4">
              Verification reviews typically take up to 48 business hours. We'll update your status as soon as possible.
            </p>
          </div>
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6 py-12 px-4 text-center">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-3xl font-extrabold tracking-tight text-red-600 dark:text-red-400">Verification Rejected</h1>
            <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mt-2">
              Your previous document submission could not be verified. This can happen if the images are blurry, cropped, or details don't match your account profiles.
            </p>
          </div>
          <div className="pt-4">
            <Button onClick={() => setKycStatus('PENDING')} size="lg" className="px-8 gap-2">
  ...
  const uploadFields = [
    { key: 'nrcFront', label: 'NRC Front Side', icon: <IdCard className="w-6 h-6" /> },
    { key: 'nrcBack', label: 'NRC Back Side', icon: <IdCard className="w-6 h-6" /> },
    { key: 'selfie', label: 'Selfie with NRC / Photo', icon: <Camera className="w-6 h-6" /> },
    { key: 'drivingLicenseFront', label: 'Driving License Front Side', icon: <FileImage className="w-6 h-6" /> },
    { key: 'drivingLicenseBack', label: 'Driving License Back Side', icon: <FileImage className="w-6 h-6" /> },
  ];

  return (
    <div className="max-w-5xl mx-auto space-y-8 px-4 py-6">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">KYC Verification</h1>
        <p className="text-slate-500 dark:text-slate-400 mt-1">Upload clear images of your documents to verify your driver credentials and unlock car bookings.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {uploadFields.map((field) => {
            ...
            const file = field.key === 'nrcFront' ? nrcFront 
                       : field.key === 'nrcBack' ? nrcBack 
                       : field.key === 'selfie' ? selfie 
                       : field.key === 'drivingLicenseFront' ? drivingLicenseFront 
                       : drivingLicenseBack;

            return (
              <Card key={field.key} className="border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-[280px]">
                ...
                <CardContent className="flex-1 flex flex-col justify-center">
                  {preview ? (
                    ...
                    <div
                      ...
                      onClick={() => fileInputRefs[field.key as 'nrcFront' | 'nrcBack' | 'selfie' | 'drivingLicenseFront' | 'drivingLicenseBack'].current?.click()}
                      className={cn(
                    ...
                    >
                      <input
                        type="file"
                        accept="image/*"
                        ref={fileInputRefs[field.key as 'nrcFront' | 'nrcBack' | 'selfie' | 'drivingLicenseFront' | 'drivingLicenseBack']}
                        onChange={(e) => {
                      ...
                      <p className="text-sm font-semibold">Upload {field.label}</p>
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          <Button 
            ...
            disabled={uploading || !nrcFront || !nrcBack || !selfie || !drivingLicenseFront || !drivingLicenseBack}
          >
            {uploading ? (
              <>
