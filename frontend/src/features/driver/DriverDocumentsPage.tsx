...
export function DriverDocumentsPage() {
  ...
  const loadDocuments = async () => {
    try {
      const data = await usersApi.getDriverDocuments()
      setDocuments(data)
    } catch {
      // handle
    } finally {
  ...
  const handleUpload = async (type: string, file: File) => {
    try {
      setUploading(type)
      const doc = await usersApi.uploadDriverDocument(type, file)
      setDocuments((prev) => [...prev.filter((d) => d.type !== type), doc])
      addToast(`${type} uploaded`, 'success')
    } catch {
              ...
              <Card>
                ...
                <CardContent>
                  {existing?.status === 'approved' ? (
                    <div className="flex items-center gap-2 text-sm text-emerald-600 p-3 bg-emerald-50 rounded-lg"><CheckCircle className="w-4 h-4" /> Approved</div>
                  ) : existing?.status === 'rejected' ? (
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 text-sm text-red-600 p-3 bg-red-50 rounded-lg"><XCircle className="w-4 h-4" /> Rejected</div>
                      {existing.admin_notes && <p className="text-xs text-muted-foreground">Reason: {existing.admin_notes}</p>}
                      <FileUploader label="Re-upload" onUpload={(file) => handleUpload(doc.key, file)} uploading={uploading === doc.key} />
                    </div>
                  ) : existing?.status === 'pending' ? (
                    <div className="flex items-center gap-2 text-sm text-yellow-600 p-3 bg-yellow-50 rounded-lg"><Clock className="w-4 h-4" /> Under review</div>
                  ) : (
                    <FileUploader label={`Upload ${doc.label}`} onUpload={(file) => handleUpload(doc.key, file)} uploading={uploading === doc.key} />
                  )}
                </CardContent>
              </Card>
            </motion.div>
